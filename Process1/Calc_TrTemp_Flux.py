...
ta = pk.load(open(Source+models+'_Ajd_4xCO2_TmSrs.pi','rb'))['ta']
tas = pk.load(open(Source+models+'_Ajd_4xCO2_TmSrs.pi','rb'))['tas']
...
for kernel in Kernel_interp_mask.keys():
    ...
    plnck_fb_tot[kernel] = np.zeros([150,64,128])
    plnck_fb_clr[kernel] = np.zeros([150,64,128])

    lpse_fb_tot[kernel] = np.zeros([150,64,128])
    lpse_fb_clr[kernel] = np.zeros([150,64,128])
