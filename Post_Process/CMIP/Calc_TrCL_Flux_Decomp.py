...
albcs_source = natsorted(
    glob.glob('/praid/users/jgvirgin/CanESM_Data/'+model+'/Raw/*_piControl_*'))[2]
...
for i in range(len(sect)):
    ...
    Klw0[sect[i]] = np.zeros([nyr_4xco2, 12, 7, sections_length[i], 64, 128])
    Klw_prime[sect[i]] = np.zeros([nyr_4xco2, 12, 7, sections_length[i], 64, 128])

    for j in range(nyr_4xco2):
...
for i in range(len(sect)):
    ...
    dRlw_true[sect[i]] = np.zeros([nyr_4xco2, 12, 64, 128])
...
for i in range(len(sect)):
    ...
    Ksw0[sect[i]] = np.zeros([nyr_4xco2, 12, 7, sections_length[i], 64, 128])
    Ksw_prime[sect[i]] = np.zeros([nyr_4xco2, 12, 7, sections_length[i], 64, 128])
...
for i in range(len(sect)):
    ...
    dRsw_true[sect[i]] = np.zeros([nyr_4xco2, 12, 64, 128])
