...
for j in range(len(fb_var)):
    var[fb_var[j]] = pk.load(open(Source+models+'_Ajd_4xCO2_TmSrs.pi','rb'))[fb_var[j]]
    if fb_var[j] == 'Alb':
...
for kernel in Kernel_interp_mask.keys():
    alb_corr[kernel] = np.zeros([150,64,128])
    ta_corr[kernel] = np.zeros([150,64,128])
    tas_corr[kernel] = np.zeros([150,64,128])
    hus_lw_corr[kernel] = np.zeros([150,64,128])
    hus_sw_corr[kernel] = np.zeros([150,64,128])
    for i in range(150):
...
DIR_corr = dict()
for kernel in Kernel_interp_mask.keys():
    DIR_corr[kernel] = np.nanmean((pk.load(open('/home/jgvirgin/Projects/CanESM/Data/'+models+'/Fluxes/'+\
                models+'_DIR_FLUXCS_Grid.pi','rb'))[kernel])-\
                (pk.load(open('/home/jgvirgin/Projects/CanESM/Data/'+models+'/Fluxes/'+\
...
CRE_lw = pk.load(open('/home/jgvirgin/Projects/CanESM/Data/'+models+'/Fluxes/'+models+'_TrCRElw_FLUX_Grid.pi','rb'))
CRE_sw = pk.load(open('/home/jgvirgin/Projects/CanESM/Data/'+models+'/Fluxes/'+models+'_TrCREsw_FLUX_Grid.pi','rb'))
