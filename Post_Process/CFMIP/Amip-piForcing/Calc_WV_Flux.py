...
dhus = pk.load(open(Source+models+'_Ajd_amip-piControl_TmSrs.pi', 'rb'))['hus']
...
for kernel in Kernels_interpolated.keys():
    ...
    WV_fb_strato_tot[kernel] = np.zeros([145,64,128])
    WV_fb_strato_clr[kernel] = np.zeros([145,64,128])

    WV_fb_tropo_tot[kernel] = np.zeros([145,64,128])
    WV_fb_tropo_clr[kernel] = np.zeros([145,64,128])
